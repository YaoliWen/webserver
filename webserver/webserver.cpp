...
WebServer::WebServer(
    ...
    port_(port),timeoutMS_(timeoutMS),isClose_(false),openLinger_(optLinger),
    timer_(new TimerManager()),threadpool_(new ThreadPool(threadNum)),epoller_(new Epoller())
{
    ...
    // /home/U0/CODE/WebServer-master/WebServer-master/resources
    // printf("%s", srcDir_);
    ...
    while(!isClose_)
    {
        ...
        for(int i=0;i<eventCnt;++i)
        {
            ...
            if(fd==listenFd_)
            {
                ...
                //  std::cout<<fd<<" close connect"<<std::endl;
            }
                ...
                // std::cout<<fd<<" writing end!"<<std::endl;
            } 
            else {
